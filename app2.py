...
def world_map(df):
    fig = px.choropleth(df, locations="iso_alpha", color = "TotalConfirmed",
                        hover_name= "Country",
                        hover_data = ['TotalConfirmed','TotalDeaths'],
        ...
        dbc.CardBody(
            [
               dcc.Markdown( dangerously_allow_html = True,
                   children = ["{0} <br><sub>+{0}</sub></br>".format(total_cases)])
...
body_app = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(dbc.Card(data_for_cases("Confirmed",f'{confirmed:,}' ), color = 'primary',style = {'text-align':'center'}, inverse = True),xs = 12, sm = 12, md = 4, lg = 4, xl = 4, ),
        dbc.Col(dbc.Card(data_for_cases("Deaths",f'{deaths:,}' ), color = 'danger',style = {'text-align':'center'}, inverse = True),xs = 12, sm = 12, md = 4, lg = 4, xl = 4,)
         ]),
...
navbar = dbc.Navbar( id = 'navbar', children = [
    ...
    href = '/'
        ...
        # dbc.Button(id = 'button', children = "Click Me!", color = "primary"), 
            dbc.Button(id = 'button', children = "Support Us", color = "primary", className = "g-0 ms-auto flex-nowrap mt-3 mt-md-0", href = '/')
...
def table_country(country):
    ...
    columns = [{'id':c , 'name':c} for c in df_final[['Country','TotalConfirmed','TotalDeaths']].columns],
